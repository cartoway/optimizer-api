  ...
  class Dichotomous
    ...
    def self.set_config(service_vrp) # rubocop: disable Naming/AccessorMethodName, Style/CommentedKeyword
      ...
      config.resolution.init_duration =
        if (service_vrp.dicho_sides.nil? || !service_vrp.dicho_sides[:cannot_split_further]) &&
    ...
    def self.split(service_vrp, job = nil)
      ...
      unless service_vrp.dicho_sides
        service_vrp.dicho_sides, _empties_or_fills = SplitClustering.initialize_split_data(service_vrp, job)
      end
      dicho_data = service_vrp.dicho_sides
